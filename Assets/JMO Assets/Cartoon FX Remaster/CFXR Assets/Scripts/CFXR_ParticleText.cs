...
#pragma warning restore 0649
        ...
        private void InitializeFirstParticle( ) {
            ...
            ParticleSystem ps = isDynamic ? this.transform.GetChild( 0 ).GetComponent<ParticleSystem>( ) : this.GetComponent<ParticleSystem>( );
            ...
            if ( charCount > 0 ) {
                ...
                if ( childCount < charCount ) {
                    ...
                    for ( int i = childCount; i < charCount; i++ ) {
                        GameObject newLetter = isDynamic ? Instantiate( model, this.transform ) : new GameObject( );
                        if ( !isDynamic ) {
                            newLetter.transform.SetParent( this.transform );
                            newLetter.AddComponent<ParticleSystem>( );
                        }

                        newLetter.transform.SetLocalPositionAndRotation( Vector3.zero, Quaternion.identity );
                    }
                }
                ...
                ParticleSystem sourceParticle = isDynamic ? null : this.GetComponent<ParticleSystem>( );
                ParticleSystemRenderer sourceParticleRenderer = this.GetComponent<ParticleSystemRenderer>( );
                ...
                for ( int i = 0; i < text.Length; i++ ) {
                    ...
                    if ( char.IsWhiteSpace( letter ) ) {
                        ...
                        GameObject letterObj = this.transform.GetChild( currentChild ).gameObject;
